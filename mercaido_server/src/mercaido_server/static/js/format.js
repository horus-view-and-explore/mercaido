// src/_lib/requiredArgs/index.ts
...
// src/_lib/getUTCDayOfYear/index.ts
...
// src/_lib/startOfUTCISOWeek/index.ts
...
// src/_lib/getUTCISOWeekYear/index.ts
function getUTCISOWeekYear(dirtyDate) {
  requiredArgs(1, arguments);
  const date = toDate(dirtyDate);
  const year = date.getUTCFullYear();
  const fourthOfJanuaryOfNextYear = /* @__PURE__ */ new Date(0);
  fourthOfJanuaryOfNextYear.setUTCFullYear(year + 1, 0, 4);
  fourthOfJanuaryOfNextYear.setUTCHours(0, 0, 0, 0);
  const startOfNextYear = startOfUTCISOWeek(fourthOfJanuaryOfNextYear);
  const fourthOfJanuaryOfThisYear = /* @__PURE__ */ new Date(0);
...
// src/_lib/startOfUTCISOWeekYear/index.ts
function startOfUTCISOWeekYear(dirtyDate) {
  requiredArgs(1, arguments);
  const year = getUTCISOWeekYear(dirtyDate);
  const fourthOfJanuary = /* @__PURE__ */ new Date(0);
...
// src/_lib/getUTCISOWeek/index.ts
var MILLISECONDS_IN_WEEK = 6048e5;
function getUTCISOWeek(dirtyDate) {
  requiredArgs(1, arguments);
  const date = toDate(dirtyDate);
  const diff = startOfUTCISOWeek(date).getTime() - startOfUTCISOWeekYear(date).getTime();
...
// src/_lib/startOfUTCWeek/index.ts
function startOfUTCWeek(dirtyDate, options) {
  requiredArgs(1, arguments);
  const defaultOptions2 = getDefaultOptions();
  const weekStartsOn = toInteger(
    options?.weekStartsOn ?? options?.locale?.options?.weekStartsOn ?? defaultOptions2.weekStartsOn ?? defaultOptions2.locale?.options?.weekStartsOn ?? 0
  );
...
// src/_lib/getUTCWeekYear/index.ts
function getUTCWeekYear(dirtyDate, options) {
  ...
  const firstWeekContainsDate = toInteger(
    options?.firstWeekContainsDate ?? options?.locale?.options?.firstWeekContainsDate ?? defaultOptions2.firstWeekContainsDate ?? defaultOptions2.locale?.options?.firstWeekContainsDate ?? 1
  );
  ...
  const firstWeekOfNextYear = /* @__PURE__ */ new Date(0);
  firstWeekOfNextYear.setUTCFullYear(year + 1, 0, firstWeekContainsDate);
  firstWeekOfNextYear.setUTCHours(0, 0, 0, 0);
  const startOfNextYear = startOfUTCWeek(firstWeekOfNextYear, options);
  const firstWeekOfThisYear = /* @__PURE__ */ new Date(0);
...
// src/_lib/startOfUTCWeekYear/index.ts
function startOfUTCWeekYear(dirtyDate, options) {
  ...
  const firstWeek = /* @__PURE__ */ new Date(0);
...
// src/_lib/getUTCWeek/index.ts
var MILLISECONDS_IN_WEEK2 = 6048e5;
function getUTCWeek(dirtyDate, options) {
  requiredArgs(1, arguments);
  const date = toDate(dirtyDate);
  const diff = startOfUTCWeek(date, options).getTime() - startOfUTCWeekYear(date, options).getTime();
...
function buildMatchFn(args) {
  return (string, options = {}) => {
    ...
    const key = Array.isArray(parsePatterns) ? findIndex(parsePatterns, (pattern) => pattern.test(matchedString)) : findKey(
      parsePatterns,
...
function format(dirtyDate, dirtyFormatStr, options) {
  ...
  const defaultOptions2 = getDefaultOptions();
  const locale2 = options?.locale ?? defaultOptions2.locale ?? defaultLocale_default;
  const firstWeekContainsDate = toInteger(
    options?.firstWeekContainsDate ?? options?.locale?.options?.firstWeekContainsDate ?? defaultOptions2.firstWeekContainsDate ?? defaultOptions2.locale?.options?.firstWeekContainsDate ?? 1
  );
  if (!(firstWeekContainsDate >= 1 && firstWeekContainsDate <= 7)) {
